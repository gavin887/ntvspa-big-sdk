...
use Ntvspa\Big\Product\Structs\PaymentField;
...
class ArrayOfPaymentField extends AbstractStructArrayBase
{
    ...
    public static function validatePaymentFieldForArrayConstraintFromSetPaymentField(?array $values = []): string
    {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The PaymentField property can only contain items of type \Ntvspa\Big\Product\Structs\PaymentField, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     */
    public function current(): ?\Ntvspa\Big\Product\Structs\PaymentField
    {
     ...
     */
    public function item($index): ?\Ntvspa\Big\Product\Structs\PaymentField
    {
     ...
     */
    public function first(): ?\Ntvspa\Big\Product\Structs\PaymentField
    {
        return parent::first();
    }
     ...
     */
    public function last(): ?\Ntvspa\Big\Product\Structs\PaymentField
    {
        return parent::last();
    }
     ...
     */
    public function offsetGet($offset): ?\Ntvspa\Big\Product\Structs\PaymentField
    {
    ...
    public function add($item): self
    {
        ...
        if (!$item instanceof PaymentField) {
            throw new InvalidArgumentException(sprintf('The PaymentField property can only contain items of type \Ntvspa\Big\Product\Structs\PaymentField, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
