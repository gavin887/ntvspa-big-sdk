...
use Ntvspa\Big\Product\Structs\Booking;
...
class ArrayOfBooking extends AbstractStructArrayBase
{
    ...
    public static function validateBookingForArrayConstraintFromSetBooking(?array $values = []): string
    {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Booking property can only contain items of type \Ntvspa\Big\Product\Structs\Booking, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     */
    public function current(): ?\Ntvspa\Big\Product\Structs\Booking
    {
     ...
     */
    public function item($index): ?\Ntvspa\Big\Product\Structs\Booking
    {
     ...
     */
    public function first(): ?\Ntvspa\Big\Product\Structs\Booking
    {
        return parent::first();
    }
     ...
     */
    public function last(): ?\Ntvspa\Big\Product\Structs\Booking
    {
        return parent::last();
    }
     ...
     */
    public function offsetGet($offset): ?\Ntvspa\Big\Product\Structs\Booking
    {
    ...
    public function add($item): self
    {
        ...
        if (!$item instanceof Booking) {
            throw new InvalidArgumentException(sprintf('The Booking property can only contain items of type \Ntvspa\Big\Product\Structs\Booking, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
