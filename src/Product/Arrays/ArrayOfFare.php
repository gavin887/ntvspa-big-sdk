...
use Ntvspa\Big\Product\Structs\Fare;
...
class ArrayOfFare extends AbstractStructArrayBase
{
    ...
    public static function validateFareForArrayConstraintFromSetFare(?array $values = []): string
    {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Fare property can only contain items of type \Ntvspa\Big\Product\Structs\Fare, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     */
    public function current(): ?\Ntvspa\Big\Product\Structs\Fare
    {
     ...
     */
    public function item($index): ?\Ntvspa\Big\Product\Structs\Fare
    {
     ...
     */
    public function first(): ?\Ntvspa\Big\Product\Structs\Fare
    {
        return parent::first();
    }
     ...
     */
    public function last(): ?\Ntvspa\Big\Product\Structs\Fare
    {
        return parent::last();
    }
     ...
     */
    public function offsetGet($offset): ?\Ntvspa\Big\Product\Structs\Fare
    {
    ...
    public function add($item): self
    {
        ...
        if (!$item instanceof Fare) {
            throw new InvalidArgumentException(sprintf('The Fare property can only contain items of type \Ntvspa\Big\Product\Structs\Fare, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
