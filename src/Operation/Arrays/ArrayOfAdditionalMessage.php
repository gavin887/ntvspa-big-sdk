...
use Ntvspa\Big\Operation\Structs\AdditionalMessage;
...
class ArrayOfAdditionalMessage extends AbstractStructArrayBase
{
    ...
    public static function validateAdditionalMessageForArrayConstraintFromSetAdditionalMessage(?array $values = []): string
    {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The AdditionalMessage property can only contain items of type \Ntvspa\Big\Operation\Structs\AdditionalMessage, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     */
    public function current(): ?\Ntvspa\Big\Operation\Structs\AdditionalMessage
    {
     ...
     */
    public function item($index): ?\Ntvspa\Big\Operation\Structs\AdditionalMessage
    {
     ...
     */
    public function first(): ?\Ntvspa\Big\Operation\Structs\AdditionalMessage
    {
        return parent::first();
    }
     ...
     */
    public function last(): ?\Ntvspa\Big\Operation\Structs\AdditionalMessage
    {
        return parent::last();
    }
     ...
     */
    public function offsetGet($offset): ?\Ntvspa\Big\Operation\Structs\AdditionalMessage
    {
    ...
    public function add($item): self
    {
        ...
        if (!$item instanceof AdditionalMessage) {
            throw new InvalidArgumentException(sprintf('The AdditionalMessage property can only contain items of type \Ntvspa\Big\Operation\Structs\AdditionalMessage, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
