...
use Ntvspa\Big\Operation\Structs\Warning;
...
class ArrayOfWarning extends AbstractStructArrayBase
{
    ...
    public static function validateWarningForArrayConstraintFromSetWarning(?array $values = []): string
    {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Warning property can only contain items of type \Ntvspa\Big\Operation\Structs\Warning, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     */
    public function current(): ?\Ntvspa\Big\Operation\Structs\Warning
    {
     ...
     */
    public function item($index): ?\Ntvspa\Big\Operation\Structs\Warning
    {
     ...
     */
    public function first(): ?\Ntvspa\Big\Operation\Structs\Warning
    {
        return parent::first();
    }
     ...
     */
    public function last(): ?\Ntvspa\Big\Operation\Structs\Warning
    {
        return parent::last();
    }
     ...
     */
    public function offsetGet($offset): ?\Ntvspa\Big\Operation\Structs\Warning
    {
    ...
    public function add($item): self
    {
        ...
        if (!$item instanceof Warning) {
            throw new InvalidArgumentException(sprintf('The Warning property can only contain items of type \Ntvspa\Big\Operation\Structs\Warning, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
