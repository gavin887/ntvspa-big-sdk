...
use Ntvspa\Big\Booking\Structs\LegSSR;
...
class ArrayOfLegSSR extends AbstractStructArrayBase
{
    ...
    public static function validateLegSSRForArrayConstraintFromSetLegSSR(?array $values = []): string
    {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The LegSSR property can only contain items of type \Ntvspa\Big\Booking\Structs\LegSSR, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     */
    public function current(): ?\Ntvspa\Big\Booking\Structs\LegSSR
    {
     ...
     */
    public function item($index): ?\Ntvspa\Big\Booking\Structs\LegSSR
    {
     ...
     */
    public function first(): ?\Ntvspa\Big\Booking\Structs\LegSSR
    {
        return parent::first();
    }
     ...
     */
    public function last(): ?\Ntvspa\Big\Booking\Structs\LegSSR
    {
        return parent::last();
    }
     ...
     */
    public function offsetGet($offset): ?\Ntvspa\Big\Booking\Structs\LegSSR
    {
    ...
    public function add($item): self
    {
        ...
        if (!$item instanceof LegSSR) {
            throw new InvalidArgumentException(sprintf('The LegSSR property can only contain items of type \Ntvspa\Big\Booking\Structs\LegSSR, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
