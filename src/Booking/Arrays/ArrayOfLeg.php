...
use Ntvspa\Big\Booking\Structs\Leg;
...
class ArrayOfLeg extends AbstractStructArrayBase
{
    ...
    public static function validateLegForArrayConstraintFromSetLeg(?array $values = []): string
    {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Leg property can only contain items of type \Ntvspa\Big\Booking\Structs\Leg, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     */
    public function current(): ?\Ntvspa\Big\Booking\Structs\Leg
    {
     ...
     */
    public function item($index): ?\Ntvspa\Big\Booking\Structs\Leg
    {
     ...
     */
    public function first(): ?\Ntvspa\Big\Booking\Structs\Leg
    {
        return parent::first();
    }
     ...
     */
    public function last(): ?\Ntvspa\Big\Booking\Structs\Leg
    {
        return parent::last();
    }
     ...
     */
    public function offsetGet($offset): ?\Ntvspa\Big\Booking\Structs\Leg
    {
    ...
    public function add($item): self
    {
        ...
        if (!$item instanceof Leg) {
            throw new InvalidArgumentException(sprintf('The Leg property can only contain items of type \Ntvspa\Big\Booking\Structs\Leg, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
