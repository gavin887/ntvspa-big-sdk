...
use Ntvspa\Big\Booking\Structs\Journey;
...
class ArrayOfJourney extends AbstractStructArrayBase
{
    ...
    public static function validateJourneyForArrayConstraintFromSetJourney(?array $values = []): string
    {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Journey property can only contain items of type \Ntvspa\Big\Booking\Structs\Journey, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     */
    public function current(): ?\Ntvspa\Big\Booking\Structs\Journey
    {
     ...
     */
    public function item($index): ?\Ntvspa\Big\Booking\Structs\Journey
    {
     ...
     */
    public function first(): ?\Ntvspa\Big\Booking\Structs\Journey
    {
        return parent::first();
    }
     ...
     */
    public function last(): ?\Ntvspa\Big\Booking\Structs\Journey
    {
        return parent::last();
    }
     ...
     */
    public function offsetGet($offset): ?\Ntvspa\Big\Booking\Structs\Journey
    {
    ...
    public function add($item): self
    {
        ...
        if (!$item instanceof Journey) {
            throw new InvalidArgumentException(sprintf('The Journey property can only contain items of type \Ntvspa\Big\Booking\Structs\Journey, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
