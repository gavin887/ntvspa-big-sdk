...
use Ntvspa\Big\Booking\Structs\Segment;
...
class ArrayOfSegment extends AbstractStructArrayBase
{
    ...
    public static function validateSegmentForArrayConstraintFromSetSegment(?array $values = []): string
    {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Segment property can only contain items of type \Ntvspa\Big\Booking\Structs\Segment, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     */
    public function current(): ?\Ntvspa\Big\Booking\Structs\Segment
    {
     ...
     */
    public function item($index): ?\Ntvspa\Big\Booking\Structs\Segment
    {
     ...
     */
    public function first(): ?\Ntvspa\Big\Booking\Structs\Segment
    {
        return parent::first();
    }
     ...
     */
    public function last(): ?\Ntvspa\Big\Booking\Structs\Segment
    {
        return parent::last();
    }
     ...
     */
    public function offsetGet($offset): ?\Ntvspa\Big\Booking\Structs\Segment
    {
    ...
    public function add($item): self
    {
        ...
        if (!$item instanceof Segment) {
            throw new InvalidArgumentException(sprintf('The Segment property can only contain items of type \Ntvspa\Big\Booking\Structs\Segment, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
