...
use Ntvspa\Big\Travel\Structs\Passenger;
...
class ArrayOfPassenger extends AbstractStructArrayBase
{
    ...
    public static function validatePassengerForArrayConstraintFromSetPassenger(?array $values = []): string
    {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Passenger property can only contain items of type \Ntvspa\Big\Travel\Structs\Passenger, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     */
    public function current(): ?\Ntvspa\Big\Travel\Structs\Passenger
    {
     ...
     */
    public function item($index): ?\Ntvspa\Big\Travel\Structs\Passenger
    {
     ...
     */
    public function first(): ?\Ntvspa\Big\Travel\Structs\Passenger
    {
        return parent::first();
    }
     ...
     */
    public function last(): ?\Ntvspa\Big\Travel\Structs\Passenger
    {
        return parent::last();
    }
     ...
     */
    public function offsetGet($offset): ?\Ntvspa\Big\Travel\Structs\Passenger
    {
    ...
    public function add($item): self
    {
        ...
        if (!$item instanceof Passenger) {
            throw new InvalidArgumentException(sprintf('The Passenger property can only contain items of type \Ntvspa\Big\Travel\Structs\Passenger, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
