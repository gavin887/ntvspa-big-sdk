...
use Ntvspa\Big\Travel\Structs\JourneyDateMarket;
...
class ArrayOfJourneyDateMarket extends AbstractStructArrayBase
{
    ...
    public static function validateJourneyDateMarketForArrayConstraintFromSetJourneyDateMarket(?array $values = []): string
    {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The JourneyDateMarket property can only contain items of type \Ntvspa\Big\Travel\Structs\JourneyDateMarket, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     */
    public function current(): ?\Ntvspa\Big\Travel\Structs\JourneyDateMarket
    {
     ...
     */
    public function item($index): ?\Ntvspa\Big\Travel\Structs\JourneyDateMarket
    {
     ...
     */
    public function first(): ?\Ntvspa\Big\Travel\Structs\JourneyDateMarket
    {
        return parent::first();
    }
     ...
     */
    public function last(): ?\Ntvspa\Big\Travel\Structs\JourneyDateMarket
    {
        return parent::last();
    }
     ...
     */
    public function offsetGet($offset): ?\Ntvspa\Big\Travel\Structs\JourneyDateMarket
    {
    ...
    public function add($item): self
    {
        ...
        if (!$item instanceof JourneyDateMarket) {
            throw new InvalidArgumentException(sprintf('The JourneyDateMarket property can only contain items of type \Ntvspa\Big\Travel\Structs\JourneyDateMarket, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
