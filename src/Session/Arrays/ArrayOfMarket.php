...
use Ntvspa\Big\Session\Structs\Market;
...
class ArrayOfMarket extends AbstractStructArrayBase
{
    ...
    public static function validateMarketForArrayConstraintFromSetMarket(?array $values = []): string
    {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Market property can only contain items of type \Ntvspa\Big\Session\Structs\Market, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     */
    public function current(): ?\Ntvspa\Big\Session\Structs\Market
    {
     ...
     */
    public function item($index): ?\Ntvspa\Big\Session\Structs\Market
    {
     ...
     */
    public function first(): ?\Ntvspa\Big\Session\Structs\Market
    {
        return parent::first();
    }
     ...
     */
    public function last(): ?\Ntvspa\Big\Session\Structs\Market
    {
        return parent::last();
    }
     ...
     */
    public function offsetGet($offset): ?\Ntvspa\Big\Session\Structs\Market
    {
    ...
    public function add($item): self
    {
        ...
        if (!$item instanceof Market) {
            throw new InvalidArgumentException(sprintf('The Market property can only contain items of type \Ntvspa\Big\Session\Structs\Market, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
