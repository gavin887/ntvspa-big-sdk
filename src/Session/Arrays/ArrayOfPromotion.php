...
use Ntvspa\Big\Session\Structs\Promotion;
...
class ArrayOfPromotion extends AbstractStructArrayBase
{
    ...
    public static function validatePromotionForArrayConstraintFromSetPromotion(?array $values = []): string
    {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Promotion property can only contain items of type \Ntvspa\Big\Session\Structs\Promotion, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     */
    public function current(): ?\Ntvspa\Big\Session\Structs\Promotion
    {
     ...
     */
    public function item($index): ?\Ntvspa\Big\Session\Structs\Promotion
    {
     ...
     */
    public function first(): ?\Ntvspa\Big\Session\Structs\Promotion
    {
        return parent::first();
    }
     ...
     */
    public function last(): ?\Ntvspa\Big\Session\Structs\Promotion
    {
        return parent::last();
    }
     ...
     */
    public function offsetGet($offset): ?\Ntvspa\Big\Session\Structs\Promotion
    {
    ...
    public function add($item): self
    {
        ...
        if (!$item instanceof Promotion) {
            throw new InvalidArgumentException(sprintf('The Promotion property can only contain items of type \Ntvspa\Big\Session\Structs\Promotion, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
