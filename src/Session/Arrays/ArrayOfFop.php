...
use Ntvspa\Big\Session\Structs\Fop;
...
class ArrayOfFop extends AbstractStructArrayBase
{
    ...
    public static function validateFopForArrayConstraintFromSetFop(?array $values = []): string
    {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Fop property can only contain items of type \Ntvspa\Big\Session\Structs\Fop, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     */
    public function current(): ?\Ntvspa\Big\Session\Structs\Fop
    {
     ...
     */
    public function item($index): ?\Ntvspa\Big\Session\Structs\Fop
    {
     ...
     */
    public function first(): ?\Ntvspa\Big\Session\Structs\Fop
    {
        return parent::first();
    }
     ...
     */
    public function last(): ?\Ntvspa\Big\Session\Structs\Fop
    {
        return parent::last();
    }
     ...
     */
    public function offsetGet($offset): ?\Ntvspa\Big\Session\Structs\Fop
    {
    ...
    public function add($item): self
    {
        ...
        if (!$item instanceof Fop) {
            throw new InvalidArgumentException(sprintf('The Fop property can only contain items of type \Ntvspa\Big\Session\Structs\Fop, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
