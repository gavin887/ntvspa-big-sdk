...
use Ntvspa\Big\Corporate\Structs\JourneyPromotionApplicable;
...
class ArrayOfJourneyPromotionApplicable extends AbstractStructArrayBase
{
    ...
    public static function validateJourneyPromotionApplicableForArrayConstraintFromSetJourneyPromotionApplicable(?array $values = []): string
    {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The JourneyPromotionApplicable property can only contain items of type \Ntvspa\Big\Corporate\Structs\JourneyPromotionApplicable, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     */
    public function current(): ?\Ntvspa\Big\Corporate\Structs\JourneyPromotionApplicable
    {
     ...
     */
    public function item($index): ?\Ntvspa\Big\Corporate\Structs\JourneyPromotionApplicable
    {
     ...
     */
    public function first(): ?\Ntvspa\Big\Corporate\Structs\JourneyPromotionApplicable
    {
        return parent::first();
    }
     ...
     */
    public function last(): ?\Ntvspa\Big\Corporate\Structs\JourneyPromotionApplicable
    {
        return parent::last();
    }
     ...
     */
    public function offsetGet($offset): ?\Ntvspa\Big\Corporate\Structs\JourneyPromotionApplicable
    {
    ...
    public function add($item): self
    {
        ...
        if (!$item instanceof JourneyPromotionApplicable) {
            throw new InvalidArgumentException(sprintf('The JourneyPromotionApplicable property can only contain items of type \Ntvspa\Big\Corporate\Structs\JourneyPromotionApplicable, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
