...
use Ntvspa\Big\Customer\Structs\Contact;
...
class ArrayOfContact extends AbstractStructArrayBase
{
    ...
    public static function validateContactForArrayConstraintFromSetContact(?array $values = []): string
    {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Contact property can only contain items of type \Ntvspa\Big\Customer\Structs\Contact, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     */
    public function current(): ?\Ntvspa\Big\Customer\Structs\Contact
    {
     ...
     */
    public function item($index): ?\Ntvspa\Big\Customer\Structs\Contact
    {
     ...
     */
    public function first(): ?\Ntvspa\Big\Customer\Structs\Contact
    {
        return parent::first();
    }
     ...
     */
    public function last(): ?\Ntvspa\Big\Customer\Structs\Contact
    {
        return parent::last();
    }
     ...
     */
    public function offsetGet($offset): ?\Ntvspa\Big\Customer\Structs\Contact
    {
    ...
    public function add($item): self
    {
        ...
        if (!$item instanceof Contact) {
            throw new InvalidArgumentException(sprintf('The Contact property can only contain items of type \Ntvspa\Big\Customer\Structs\Contact, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
