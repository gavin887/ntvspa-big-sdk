...
use Ntvspa\Big\Customer\Structs\Property;
...
class ArrayOfProperty extends AbstractStructArrayBase
{
    ...
    public static function validatePropertyForArrayConstraintFromSetProperty(?array $values = []): string
    {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Property property can only contain items of type \Ntvspa\Big\Customer\Structs\Property, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     */
    public function current(): ?\Ntvspa\Big\Customer\Structs\Property
    {
     ...
     */
    public function item($index): ?\Ntvspa\Big\Customer\Structs\Property
    {
     ...
     */
    public function first(): ?\Ntvspa\Big\Customer\Structs\Property
    {
        return parent::first();
    }
     ...
     */
    public function last(): ?\Ntvspa\Big\Customer\Structs\Property
    {
        return parent::last();
    }
     ...
     */
    public function offsetGet($offset): ?\Ntvspa\Big\Customer\Structs\Property
    {
    ...
    public function add($item): self
    {
        ...
        if (!$item instanceof Property) {
            throw new InvalidArgumentException(sprintf('The Property property can only contain items of type \Ntvspa\Big\Customer\Structs\Property, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
